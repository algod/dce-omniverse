      ...
      <div className="bg-white rounded-lg p-4" style={{ height: '400px' }}>
        ...
        {selectedView === 'sequences' && (
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto space-y-3" style={{ maxHeight: 'calc(100% - 2rem)' }}>
              {channelSequence.map((seq, index) => (
                <div key={index} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2 flex-1">
                      <Network size={16} className="text-blue-500 flex-shrink-0" />
                      <span className="text-sm font-medium text-gray-900 truncate">{seq.sequence}</span>
                    </div>
                    <span className={`px-2 py-1 text-xs rounded-full font-medium whitespace-nowrap ml-2 ${
